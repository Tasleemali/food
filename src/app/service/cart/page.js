            ...
            {cartItems.map((item) => (
              <Card key={item.id} className="p-4">
                <CardHeader className="flex justify-between items-center">
                  <Image src={item.image} className=' h-10 w-10  md:h-28 md:w-28'/>
                  <CardTitle>{item.name}</CardTitle>
                  <div className="flex items-center space-x-2">
                    <Button variant="outline" size="icon"  onClick={()=>{ item.qty > 1 ? reversecart(item) :null}}   >
                      <Minus className="h-4 w-4" />
                    </Button>
                    <span className="text-lg font-bold">{item.qty}</span>
                    <Button variant="outline" size="icon" onClick={()=> addcart(item)} >
                      <Plus className="h-4 w-4" />
                    </Button>
                  </div>
                 
                  <div>
                  <Button variant="outline" size="icon" onClick={()=> removecart(item._id)} >
                    X
                    </Button>
                  </div>
                </CardHeader>
